...
export default function ModelLabPage() {
  ...
  useEffect(() => {
    fetchModelInfo()
      .then(setModelInfo)
      .catch((err) => setInfoError(err.message || "Could not load model metadata."));
  }, []);
  ...
  async function runPrediction() {
    ...
    try {
      ...
      setRunError(err.message || "Prediction request failed.");
    } finally {
      setRunning(false);
    }
  }

  if (loading) return <LoadingBlock label="Loading AI Model Lab..." />;
  if (loadError) return <ErrorBlock message={loadError} />;

  return (
    <div>
      <p className="section-sub">
        A transparent look at the prototype machine learning models behind HydroMind AI's health
        and risk predictions — trained on synthetic hydroponic data, with a rule-based safety
        layer that can override the model.
      </p>
      ...
      {modelInfo && (
        <>
          <div className="grid grid-cols-4">
            <InfoTile icon={Database} label="Dataset Size" value={modelInfo.dataset_size} sub={`${modelInfo.train_size} train / ${modelInfo.test_size} test`} />
            <InfoTile icon={Cpu} label="Model Type" value="Random Forest" sub={`${modelInfo.health_model.n_estimators} trees per model`} />
            <InfoTile icon={Target} label="Health Accuracy" value={`${Math.round(modelInfo.health_model.accuracy * 100)}%`} sub="On held-out synthetic test set" />
            <InfoTile icon={CalendarClock} label="Last Trained" value={new Date(modelInfo.trained_at).toLocaleDateString("en-US")} sub={new Date(modelInfo.trained_at).toLocaleTimeString("en-US")} />
          </div>

          <div className="grid grid-cols-2 mt-16">
            <div className="card">
              <h3 style={{ marginTop: 0, fontSize: 14 }}>Health Status Classifier</h3>
              <div className="kv-row">
                <span className="kv-label">Type</span>
                <span className="kv-value">{modelInfo.health_model.type}</span>
              </div>
              <div className="kv-row">
                <span className="kv-label">Classes</span>
                <span className="kv-value">{modelInfo.health_classes.join(", ")}</span>
              </div>
              <div className="kv-row">
                <span className="kv-label">Accuracy (test set)</span>
                <span className="kv-value">{(modelInfo.health_model.accuracy * 100).toFixed(1)}%</span>
              </div>
              <div className="kv-row">
                <span className="kv-label">Training status</span>
                <span className="badge badge-good">Trained</span>
              </div>
            </div>
            <div className="card">
              <h3 style={{ marginTop: 0, fontSize: 14 }}>Risk Score Regressor</h3>
              <div className="kv-row">
                <span className="kv-label">Type</span>
                <span className="kv-value">{modelInfo.risk_model.type}</span>
              </div>
              <div className="kv-row">
                <span className="kv-label">R² (test set)</span>
                <span className="kv-value">{modelInfo.risk_model.r2_score}</span>
              </div>
              <div className="kv-row">
                <span className="kv-label">Mean Absolute Error</span>
                <span className="kv-value">{modelInfo.risk_model.mean_absolute_error} pts</span>
              </div>
              <div className="kv-row">
                <span className="kv-label">Training status</span>
                <span className="badge badge-good">Trained</span>
          ...
          <div className="card mt-16">
            <h3 style={{ marginTop: 0, fontSize: 14 }}>Features Used</h3>
      ...
      <div className="card mt-24" style={{ borderColor: "var(--brand-blue)" }}>
        <div className="card-title-row">
          <h3 className="flex items-center gap-8">
            ...
            Try It — Live Prediction
          </h3>
        </div>
        <p className="text-secondary" style={{ fontSize: 12.5, marginTop: 0 }}>
          Pick a crop, stage, and sensor values, then run them through the trained models to see
          a live prediction.
        </p>
        ...
        <div className="grid grid-cols-3 mt-16">
          ...
          <label className="flex flex-col gap-6" style={{ fontSize: 12 }}>
            <span className="text-muted">Hydroponic Method</span>
        ...
        {form && (
          <div className="grid grid-cols-4 mt-16">
            <NumberField label="pH" value={form.ph} onChange={(v) => setForm({ ...form, ph: v })} />
            <NumberField label="EC (mS/cm)" value={form.ec} onChange={(v) => setForm({ ...form, ec: v })} />
            <NumberField
              label="Water Temp (°C)"
          ...
          <div className="grid grid-cols-2 mt-16" style={{ alignItems: "start" }}>
            ...
            >
              ...
              <div className="flex items-center gap-8 mt-16" style={{ fontSize: 12.5 }}>
                ...
                {result.is_safety_override && <span className="badge badge-critical">Safety Override</span>}
              </div>
            </div>
            <div className="card">
              ...
              <p style={{ fontSize: 13, fontWeight: 600, margin: "6px 0 12px" }}>
                {result.detected_problem || "No problem detected."}
              </p>
              <div className="text-muted" style={{ fontSize: 12 }}>
                Recommended Action
              </div>
